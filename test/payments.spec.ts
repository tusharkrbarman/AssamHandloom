...
describe("checkout verification", () => {
  it("marks the order paid and consumes reservations on a valid callback", async () => {
    const { orderId, token } = await placeOrder();
...
describe("razorpay webhooks", () => {
  ...
  it("ignores unrelated events", async () => {
    const response = await sendWebhook(JSON.stringify({ event: "refund.processed", payload: {} }));
    expect(response.status).toBe(200);
  });
});
